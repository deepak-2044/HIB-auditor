...
export default defineConfig(({mode}) => {
  ...
  console.log('Build Environment Check:', {
    mode,
    hasGemini: !!(env.VITE_GEMINI_API_KEY || env.GEMINI_API_KEY || process.env.GEMINI_API_KEY),
    hasSupabaseUrl: !!(env.VITE_SUPABASE_URL || env.SUPABASE_URL || process.env.SUPABASE_URL),
    hasSupabaseKey: !!(env.VITE_SUPABASE_ANON_KEY || env.SUPABASE_ANON_KEY || process.env.SUPABASE_ANON_KEY)
  });
  ...
  return {
    ...
    define: {
      ...
      'process.env.VITE_SUPABASE_ANON_KEY': JSON.stringify(env.VITE_SUPABASE_ANON_KEY || env.SUPABASE_ANON_KEY || process.env.SUPABASE_ANON_KEY || ""),
      // Fallbacks for direct process.env usage
